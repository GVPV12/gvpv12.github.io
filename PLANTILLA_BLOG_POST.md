# Plantilla para un blog post

Usa esta plantilla para crear una nueva entrada:

1. Copia el bloque completo de abajo.
2. Guarda el archivo dentro de `src/content/blog/` con un nombre descriptivo terminado en `.md`.
3. Sustituye todos los textos entre corchetes.
4. Añade una imagen junto al archivo y descomenta `coverImage` si quieres usarla.
5. Ejecuta `pnpm build` para comprobar que el post es válido.

La etiqueta `blog` hace que la entrada aparezca en `/blog/`. Las demás etiquetas sirven para clasificarla.
